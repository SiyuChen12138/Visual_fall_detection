...
> 因此整体以 GPL-3.0 发布。第三方组件与版权归属详见 [NOTICE.md](NOTICE.md)，数据集版权见 [docs/DATASET_LICENSE.md](docs/DATASET_LICENSE.md)。
...
├── NOTICE.md                   # 第三方组件与许可声明（必读）
├── docs/
│   └── DATASET_LICENSE.md      # 数据集来源与许可说明
├── weights/
│   └── yolov5s.pt              # 官方预训练权重（训练起点）
├── mydata/
│   └── fall_down_yolo/
│       └── data.yaml           # 数据集配置（图片与标注不公开，见第四节）
└── runs/
    └── train/
        └── exp4/               # 最终一次训练结果
...
> ⚠️ **本仓库不包含数据集。** 训练所用数据集为作者自行购置的第三方数据集，
> **不具备公开再分发权利**，因此图片与标注文件均未上传。详见 [docs/DATASET_LICENSE.md](docs/DATASET_LICENSE.md)。
...
把数据按下面的目录结构放好，再改写 `mydata/fall_down_yolo/data.yaml` 里的路径即可：
...
- 本仓库在其基础上完成了数据集制作、模型训练、PyQt5 界面开发与 exe 打包
...
| `mydata/` | **不含数据集**，仅保留 `data.yaml`；数据集为自购，不再分发（见第四节） |
