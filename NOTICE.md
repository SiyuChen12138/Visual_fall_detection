...
| `mydata/fall_down_yolo/data.yaml` | **本项目新增**：数据集配置模板 |
| `README.md`、`NOTICE.md`、`.gitignore` | **原创**：本项目文档与配置 |

### 3. 未包含的上游内容

上游仓库中的 `.github/`（CI 工作流、Issue 模板、FUNDING / Dependabot 配置）和 `CONTRIBUTING.md`
属于 **Ultralytics 自身的仓库管理配置**，与本软件功能无关，因此未包含在本仓库中。
如需查看，请访问 [ultralytics/yolov5](https://github.com/ultralytics/yolov5)。
...
本仓库不包含训练图片与标注文件，仅保留 `mydata/fall_down_yolo/data.yaml` 用于说明数据格式。
